...
(* 
Edited by Troels C. Damgaard (TCD) 15/4/04 

Stripped Report, Crash and Edlib-dependencies:
- Report-depencies commented out (means fun reportStringTree is not available 
  (removed from SIGNATURE also)
- Crash's implemented by exceptions.
- Edlib.List func's implemented by simple funcs on regular List. 
*)

structure EdLibPrettyPrint : EDLIBPRETTYPRINT =
  struct

  val raggedRight = ref false
  val colwidth = ref 100

  fun newlinesep [] = ""
    | newlinesep (fst_string::string_list) = 
      implode( explode(fst_string) @ (foldr (fn (s,cl) => #"\n" :: explode(s) @ cl) [] string_list))


(* TCD 15/4/04 - Stripping Edlib-dependencies *)
    (* structure EdList = Edlib.List *) 

    fun removeLast list = 
	let val revlist = rev list
	in (hd revlist, rev (tl revlist)) end

(* TCD 15/4/04 - Stripping Crash-dependency *)
...
*)
    ...
    let fun loop (ind,m) = 
        case m of
          ...
          |  LINES (list) => 
	       ...
	       (* TCD 15/4/04 : Removing EdList-dependency *)
   ...
   (* change to pileChildren. Before, the print function would call
    ...
    and pileChildren(width, ind, childsep, nil): minipage = LINES nil
      ...
      | pileChildren(width, ind, RIGHT s, children) =
		...
		(* TCD 15/4/04 : Removing EdList-dependency *)
              ...
              end handle (* EdList.*) Empty (* _ *) => raise Crash "PrettyPrint.pileChildren"
	  (* TCD 15/4/04 : Removing EdList-dependency *)
	  (* TCD 15/4/04 - Stripping Crash-dependency *) 
	  (* Crash.impossible "PrettyPrint.pileChildren"*)
          end
    ...
    fun format(width: int, t: StringTree): minipage =
      if width < 3 then
	  raise Crash "PrettyPrint.format: width too small"
      (* TCD 15/4/04 - Stripping Crash-dependency *) 
	...
	newlinesep string_list
(* TCD 15/4/04 *)
(*      ListUtils.stringSep "" "" "\n" (fn x => x) string_list *)
      | flatten _ = raise Crash "PrettyPrint.flatten"
      (* TCD 15/4/04 - Stripping Crash-dependency *) 
      (* Crash.impossible "PrettyPrint.flatten" *)
...
(*
TCD 15/4/04: Stripping Report-dependency
    ...
    fun reportStringTree' width tree = 
     ...
     | _ => raise Crash "PrettyPrint.reportStringTree'"
     (* TCD 15/4/04 - Stripping Crash-dependency *) 
     (* Crash.impossible "PrettyPrint.reportStringTree'" *)
