...
Stripped Report, Crash and Edlib-dependencies from PrettyPrint-implementation:
- Report-depencies commented out (means fun reportStringTree is not available 
  (removed from SIGNATURE below also)
...
is much more efficient than `format' and `flatten'; the latter are
