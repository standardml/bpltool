...
fun dest_ppstream(ppstrm : ppstream) =
  let val PPS{consumer,linewidth,flush, ...} = ppstrm
...
fun add_string (ppstrm : ppstream) s =
...
fun add_newline (ppstrm : ppstream) =
  let val PPS{linewidth, ...} = ppstrm
  in add_break ppstrm (linewidth+2,0) end
