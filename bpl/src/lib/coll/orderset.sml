 ...
 * January 2007: Espen H�jsgaard: Added all function.
 *) 

functor OrderSet(Order : ORDERING): MONO_SET =
  struct
    structure PP = EdLibPrettyPrint
...
(*   
    ...
    fun addList [] s = s
      | addList ls s = 
	let
	  val (s' : Set, dupl : elt list) = addList1 ls [] s
	in 
...
*)
    ...
    fun foldUntil (f: elt -> 'b -> bool * 'b) (e:'b) (t:Set) : 'b =
      let
	fun fold' E a = (false, a)
	  | fold' (N(i,s1,s2,_)) a =
	    let
	      val (stop, acc) = fold' s1 a
	    in
	      if stop then (stop, acc) else
	      let
		val (stop, acc) = f i acc
	      in
		if stop then (stop, acc) else fold' s2 acc
	      end
	    end
      in
	#2 (fold' t e)
