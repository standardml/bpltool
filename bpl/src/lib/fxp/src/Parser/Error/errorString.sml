...
structure ErrorString : ErrorString =
   struct
	 ...
	 errorData2String (Vector.foldr (op ::) nil vec)
      ...
      val QUOT = "'"
      fun quoteErrorChar0 c = QUOT^errorChar2String c^QUOT
      fun quoteErrorChar c = if c=0wx0 then "entity end" else QUOT^errorChar2String c^QUOT
      fun quoteErrorData cs = QUOT^errorData2String cs^QUOT
      fun quoteErrorString s = QUOT^s^QUOT
      fun quoteErrorVector v = QUOT^errorVector2String v^QUOT
