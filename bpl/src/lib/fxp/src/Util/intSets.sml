...
structure IntSets : IntSets = 
   struct
	 ...
	 let val max = Vector.foldli
	    (fn (i,w,max) => if w=W.fromInt 0 then i else max) 0 vec
	 ...
	 let 
	    ...
	    val mask = W.fromInt 1 << (Word.fromInt (n mod wordSize))
	 in 
	    Vector.tabulate(idx+1,fn i => if i=idx then mask else W.fromInt 0):IntSet
	 end
	 ...
	 in if idx>=Vector.length vec then false 
	    else let val mask = W.fromInt 1 << (Word.fromInt (n mod wordSize))
		 in Vector.sub(vec,idx) && mask <> W.fromInt 0
		 end
	 end
	 ...
	 in 
	    if size>idx 
	       ...
	       (idx+1,fn i => if i<size then Vector.sub(vec,i) else if i=idx then mask else W.fromInt 0)
	 end
	 ...
	 let 
	    ...
	    fun doOne (w,off,yet) = 
		  ...
		  else if w&&mask=W.fromInt 0 then doit(i+1,mask<<0wx1)
		       else (off+i)::doit(i+1,mask<<0wx1)
	       in doit(0,W.fromInt 1)
	       end
	 ...
	 case Vector.length vec
	   of 0 => 0w0 
	    ...
	    | l => Word.fromInt(W.toInt(W.>>(W.+(Vector.sub(vec,0),Vector.sub(vec,l-1)),intShift)))
   end			      
