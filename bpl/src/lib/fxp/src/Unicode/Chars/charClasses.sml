...
structure CharClasses : CharClasses =
   struct 
      ...
      val max32 = Word32.notb (Word32.fromInt 0)
      ...
      fun indexMask c = let val idx = Chars.toInt(c>>0w5)
			    val mask = Word32.fromInt 1 <<< Char2Word c & 0w31
			in (idx,mask)
			end
      ...
      fun inCharClass(c,vec) = let val (idx,mask) = indexMask c
			       in mask &&& Vector.sub(vec,idx) <> Word32.fromInt 0
			       end 
	 ...
	 Array.array((Chars.toInt max-Chars.toInt min+1) div 32+1,Word32.fromInt 0):MutableClass
