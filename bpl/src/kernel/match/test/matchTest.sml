...
in
  ...
  val tests = [
    ...
    ("Matching outer name by two redex names",
     {agent = (x//[x1,x2] * idp(1)) o M2[x1,x2], redex = M2[x1,x2]},
     HAS [{context = x//[x1,x2] * idp(1), parameter = idx0}]),
    ...
    ("Matching redex link with idle name",
     {agent = y//[] || <->, redex = `[x]`},
     HAS [{context = y//[x] o `[x]`, parameter = <[x]> x//[] * <->}]),
    ...
    ("Matching redex inner and outer name with idle agent edge",
     {agent = (-/x * idp(1)) o K1[x] o <->,
      redex = (x//[x1,x2] * idp(1)) o (x2/x2 * K1[x1]) o `[x2]`},
     HAS [{context = -/x * idp(1), parameter = <[x2]> x2//[] * <->}]),
    ...
    ("Using local identity link to match",
     {agent = K10[][[x]] o (<[x]> M1[x]),
      redex= <[x]> `[x]`},
     HAS [{context = -/x o `[x]`,
           parameter = (<[x]> x//[]) o K10[][[y]] o M1[y]},
          {context = K10[][[x]],
           parameter = (<[x]> x/y o `[y]`) o M1[y]},
          {context = -/x o K10[][[y]] o (M1[y] `|` `[x]`),
           parameter = (<[x]> x//[]) o <->}]),
