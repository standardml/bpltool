package com.beepell.ui.dom.graph.actions;

import java.awt.event.ActionEvent;

import javax.swing.AbstractAction;
import javax.swing.ImageIcon;

import com.beepell.ui.dom.graph.DocumentGraph;
import com.beepell.ui.icon.IconRepository;

/**
 * @author Tim Hallwyl
 *
 */
public class ToggleElementNameAction extends AbstractAction {

    private static final long serialVersionUID = 1L;
    
    private final DocumentGraph panel;
    
    /**
     * Create a "toggle show element names" action.
     * 
     * @param panel the DocumentGraph to control.
     */
    public ToggleElementNameAction(DocumentGraph panel) {
        this.panel = panel;
        ImageIcon large = IconRepository.getIcon(IconRepository.ACTIONS, "text", IconRepository.MEDIUM);
        ImageIcon small = IconRepository.getIcon(IconRepository.ACTIONS, "text", IconRepository.TINY);

        this.putValue(LARGE_ICON_KEY, large);
        this.putValue(SMALL_ICON, small);
        this.putValue(NAME, "Toggle names");
        this.putValue(SHORT_DESCRIPTION, "Toggle element names");
        this.putValue(LONG_DESCRIPTION, "Toggle element names");

        this.setEnabled(true);
    }

    /* (non-Javadoc)
     * @see java.awt.event.ActionListener#actionPerformed(java.awt.event.ActionEvent)
     */
    @Override
    public void actionPerformed(ActionEvent e) {

        this.panel.setDrawElementNames(!this.panel.isDrawElementNames());

    }

}
